...
// Declare global Deno object type to include Electron-specific properties
declare global {
  interface Deno {
 ...
 */
export async function sendToElectron(projectNameEndingInDotLtProj: string): Promise<void> {
  ...
  if (typeof Deno.electronAPI !== 'undefined' && Deno.electronAPI?.runGame) {
    try {
      ...
      console.log(`Game execution requested via direct Electron API: ${projectNameEndingInDotLtProj}`);
      return;
    } catch (error) {
      console.warn("Direct Electron API failed, will try HTTP fallback:", error);
    }
  }
  ...
  try {
    ...
    console.log(`Game execution requested via HTTP: ${projectNameEndingInDotLtProj}`);
  } catch (error) {
    console.error("Failed to run game via HTTP:", error);
    throw new Error(`Could not communicate with Electron app: ${error.message}`);
  }
}

if (import.meta.main) {
  sendToElectron("_test.ltproj").then(() => {

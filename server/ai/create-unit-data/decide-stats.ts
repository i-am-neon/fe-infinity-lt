...
export default function decideStats(fe8Class: FE8Class): {
  ...
  const data = baseData[fe8Class] || baseData["Citizen"];

  const baseStats = { ...data.base };
  const growthRates = { ...data.growth };

  for (const stat of Object.keys(baseStats) as (keyof StatValues)[]) {
    const offset = randomInRange(-2, 2);
    if (stat === "HP") {
      baseStats[stat] = Math.max(1, baseStats[stat] + offset);
    } else {
      baseStats[stat] = Math.max(0, baseStats[stat] + offset);
    }
  }

  for (const stat of Object.keys(growthRates) as (keyof StatValues)[]) {
    const offset = randomInRange(-10, 10);
    const newVal = growthRates[stat] + offset;
    growthRates[stat] = Math.min(100, Math.max(0, newVal));
  }
